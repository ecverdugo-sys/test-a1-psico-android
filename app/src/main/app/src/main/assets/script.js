...
(() => {
  ...
  function applyThemeSelection(){
    ...
    // Persistir selección si 'Recordar' está activo
    try {
      if ((localStorage.getItem('quiz.remember /* removed */')||'off') === 'on') {
        localStorage.setItem('quiz.selectedThemes.v1', JSON.stringify(selectedThemes));
        const qSel = document.getElementById('quizChoice');
        if (qSel) localStorage.setItem('quiz.selectedQuiz.v1', qSel.value);
      }
    } catch(e) {}
...
(function(){
  ...
  function rememberThemesSetup(onoff){
    ...
    try{ localStorage.setItem('quiz.remember /* removed */', onoff); }catch(e){}
    // Persist selected quiz when remember is ON
    try{
      const qSel = document.getElementById('quizChoice');
      if(qSel){
        const saveQuiz = ()=>{ try{ localStorage.setItem('quiz.selectedQuiz.v1', qSel.value); }catch(e){} };
        qSel.removeEventListener && qSel.removeEventListener('change', qSel._saveQuizCb || (()=>{}));
        const cb = ()=>saveQuiz();
        qSel.addEventListener('change', cb);
        qSel._saveQuizCb = cb;
        // Save current immediately
        saveQuiz();
      }
    }catch(e){}
